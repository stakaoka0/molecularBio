...
#'
#' @description
#' `analyze_qPCR_ActD()` analyzes a csv file produced by QuantStudio based on
...
analyze_qPCR_ActD <- function(path_to_qPCR_res,
                         path_to_group_label,
                         out_dir_path,
  ...
  summarize_data <- raw_data %>%
    ...
    dplyr::select(Sample, Group, ActD_time, Target, Cq, `Amp Status`) %>%
    dplyr::group_by(Group, Target, ActD_time) %>%
