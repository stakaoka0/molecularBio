...
#'
#' @description
#' `plot_qPCR_ActD()` analyzes a tsv file produced by analyze_qPCR_ActD().
#'
#'
#'@param path_to_qPCR_res path to a tsv file produced by analyze_qPCR_ActD().
#'@param path_to_stats_res path to a tsv file produced by stats_qPCR_ActD().
...
plot_qPCR_ActD <- function(path_to_qPCR_res,
                      path_to_stats_res,
                      out_dir_path,
  ...
  if (plot_all == TRUE) {
    ...
    g1 <- plot_data %>%
      ...
      geom_dotplot(binaxis = "y",
                   ...
                   aes(x = ActD_time,
                       y = norm_exp,
                       group = interaction(Target,factor(ActD_time))),
                       dotsize = 0.5) +
      ...
      scale_x_continuous(breaks = time_course) +
    ...
    for (i in genes) {
      ...
      g1 <- plot_data %>%
        ...
        geom_dotplot(binaxis = "y",
                     ...
                     aes(x = ActD_time,
                         y = norm_exp,
                         group = interaction(Group, factor(ActD_time)),
                         color = factor(color, level = color_fct)),
                     dotsize = 0.5) +
        geom_hline(yintercept = 0.5, linetype="dotted") +
        coord_cartesian(ylim=c(0, max_exp*ylim_fold)) +
        ylab(paste0("Relative Expression")) +
        ggtitle(i) + xlab("Time after ActD (h)") +
        plot_theme_basic +
        scale_color_identity(labels = group_fct, guide = "legend") +
        scale_fill_identity(labels = group_fct, guide = "legend") +
        scale_x_continuous(breaks = time_course) +
