...
stats_qPCR_ActD <- function(path_to_qPCR_res,
                       out_dir_path,
  ...
  if (stats_type == "ttest") {
    ...
    for (i in genes) {
      ...
      for (j in time_course) {
        ...
        df <- qpcr_data %>%
          dplyr::filter(Target == i & ActD_time == j)
        ...
        if (1 %in% df$n | 0 %in% df$n) {
          ...
          stat_test <- stat_test %>%
            dplyr::mutate(Target = i, ActD_time = j,
                          .before = group1)
    ...
    #           left_join(dummy_test, by = c("group1", "group2"))
    #
    #         stat_test <- stat_test %>% dplyr::mutate(Target = i, ActD_time = j,
    #                                                  .before = group1)
    #
    #         merged_stat_test <- bind_rows(merged_stat_test, stat_test)
    #
